...
def build_cp_sat_candidate(request: SolverRequest, index: int) -> Candidate | None:
    ...
    overflow_terms = []
    for spec in ordered:
        overflow_x = model.new_int_var(0, max_x + 20, f"overflow_x_{spec.id}")
        overflow_y = model.new_int_var(0, max_y + 20, f"overflow_y_{spec.id}")
        ...
        overflow_terms.extend([overflow_x, overflow_y])
    ...
    return Candidate(
        ...
        notes=[
            ...
            "Model used fixed room sizes, NoOverlap2D, footprint overflow slack, shared-wall adjacency constraints, adjacency distances, and role-independent zoning penalties.",
        ],
    )
...
def adjacency_objective_terms(model: Any, specs: list[RoomSpec], x_vars: dict[str, Any], y_vars: dict[str, Any], widths: dict[str, int], heights: dict[str, int], index: int) -> list[Any]:
    ...
    for spec in specs:
        for target_key in spec.adjacency:
            targets = ([by_id[target_key]] if target_key in by_id else by_type.get(target_key, []))[:1]
...
def wall_touch_bools(model: Any, a: RoomSpec, b: RoomSpec, x_vars: dict[str, Any], y_vars: dict[str, Any], widths: dict[str, int], heights: dict[str, int], index: int) -> list[Any]:
    ...
    for label, edge_constraint, overlap_a, overlap_b in directions:
        touch = model.new_bool_var(f"touch_{label}_{a.id}_{b.id}_{index}")
